      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600">Operator-funded discount codes for your storefront</p>
          </div>
          <Button asChild>
            <Link href="/operator/coupons/new">
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {vendorCoupons.map((c) => (
                <tr key={c.id}>
                  <td className="px-4 py-3 font-mono font-medium">OP{c.code}</td>
                  <td className="px-4 py-3">Storefront-wide</td>
                  <td className="px-4 py-3">{c.discount}</td>
                  <td className="px-4 py-3">{c.expiresAt}</td>
                  <td className="px-4 py-3 text-right">{c.redemptions}</td>
                </tr>
              ))}
            </tbody>
          </table>
